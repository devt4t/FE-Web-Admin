<template>
  <div>
    ...
    <!-- START: MODAL -->
    <TruckFormModal
      :show="modals.truck.show"
      :id="modals.truck.id"
      @close="closeModals($event)"
      @refreshTable="() => getTableData('truck')"
      @snackbar="showSnackBar($event)"
    ></TruckFormModal>
    <DriverFormModal
      :show="modals.driver.show"
      :id="modals.driver.id"
      @close="closeModals($event)"
      @refreshTable="() => getTableData('driver')"
      @snackbar="showSnackBar($event)"
    ></DriverFormModal>
    <!-- END: MODAL -->
    ...
    >
        ...
        <v-expansion-panel-content>
          ...
          >
            <template v-slot:top>
              <v-row class="ma-0 align-center">
                ...
                <v-btn
                  ...
                  fab
                  small
                  @click="modals.truck.show = true"
                  v-if="$_sys.isAllowed('truck-driver-create')"
                >
            ...
            <template v-slot:item.plat_no="{ item }">
              <v-chip color="grey darken-3 white--text" class="rounded-lg">{{
                item.plat_no
              }}</v-chip>
            </template>
            <!-- Status -->
            <template v-slot:item.status="{ item }">
              <v-chip
                small
                :color="item.status == 'contract' ? 'primary' : 'orange'"
                class="white--text"
              >
            ...
            <template v-slot:item.is_active="{ item }">
              <v-chip
                :color="`${item.is_active ? 'green' : 'red'} white--text`"
              >
                <v-icon class="mr-1"
                  >mdi-truck-{{ item.is_active ? "check" : "alert" }}</v-icon
                >
            ...
            <template v-slot:item.type="{ item }">
              <p class="mb-0">
                {{ trucks.find((v) => v.name == item.type).type || "" }}
              </p>
              <small
                >{{ $store.getters.numberFormat(item.min_capacity) }} -
                {{ $store.getters.numberFormat(item.max_capacity) }}
                Bibit</small
              >
            </template>
            <!-- Actions -->
            <template v-slot:item.action="{ item }">
              <v-btn
                ...
                @click="
                  modals.truck.id = item.plat_no;
                  modals.truck.show = true;
                "
                v-if="$_sys.isAllowed('truck-driver-update')"
                ><v-icon class="mr-1">mdi-pencil-circle</v-icon> Edit</v-btn
              >
            </template>
          </v-data-table>
        </v-expansion-panel-content>
      ...
      <v-expansion-panel class="rounded-xl mt-2">
        <v-expansion-panel-header class="">
          <h3 class="">
            <v-btn fab x-small color="green white--text" class="mr-1"
              ><v-icon>mdi-card-account-details</v-icon></v-btn
            >
            Driver
          </h3>
        </v-expansion-panel-header>
          ...
          >
            ...
            <template v-slot:item.action="{ item }">
              <v-btn
                ...
                class="white--text pl-1"
                @click="
                  modals.driver.id = item.nik;
                  modals.driver.show = true;
                "
                ><v-icon class="mr-1">mdi-pencil-circle</v-icon> Edit</v-btn
              >
            </template>
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        const res = await axios.get(
          `${this.$store.getters.getApiUrl(
            `Get${this.$store.getters.capitalizeFirstLetter(type)}`
          )}`,
    ...
    closeModals(data) {
      ...
      } else if (data.type && data.val == true)
        this.modals[data.type].show = true;
    },
    showSnackBar(data) {
