<template>
  <v-dialog v-model="isOpen" width="50%">
    <template v-slot:default="{ isActive }">
      <v-card>
        <v-card-title>
          <span>Export Monitoring Populate</span>
        </v-card-title>
        ...
        <v-card-text class="farmer-assign-wrapper mt-3">
          ...
          <ValidationObserver :class="exportBy === 'mu' ? 'd-block' : 'd-none'" ref="firstForm"
            v-slot="{ handleSubmit }">
            <form @submit.prevent="handleSubmit(onSubmit)" autocomplete="off">
              <v-row>
                <v-col lg="12">
  ...
  methods: {
    ...
    getExportData(id) {
      return new Promise(async (resolve, reject) => {
        ...
        if (this.exportBy == 'ta') {
          payload = { ...payload, 'target_area': id };
        } else if (this.exportBy == 'mu') {
          payload = { ...payload, 'mu_no': id };
        } else {
          return;
        }

        this.$_api
          .get("populate-monitoring/1-to-2/list", payload)
          .then((res) => {
            return resolve(res);
          })
          .catch(() => {
    ...
    async onSubmit() {
      ...
      for (const _id of selectedData) {
        ...
        const configUrl = {
          pdf: `${this.$_config.baseUrlExport}export/monitoring-populate/pdf`,
          excel: `${this.$_config.baseUrlExport}export/monitoring-populate/excel`,
        };
        ...
        if (this.exportBy == 'ff') {
          ...
          selectedName = this.muList.find((item) => item.mu_no == _id)
            ? this.muList.find((item) => item.mu_no == _id).name
            : "";
        }
        ...
        const configFilename = {
          pdf: `Report-Monitoring-Populate-${selectedName}-${moment().format(
            "DMMYYYYHHmmss"
          )}.pdf`,
          excel: `Report-Monitoring-Populate-${selectedName}-${moment().format(
            "DMMYYYYHHmmss"
          )}.xlsx`,
        };
        const axiosConfig = {
