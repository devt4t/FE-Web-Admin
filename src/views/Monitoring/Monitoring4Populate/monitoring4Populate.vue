<template>
    <geko-base-crud :config="config" :refreshKey="refreshKey" :hideUpdate="true" :hideDelete="true" :hideCreate="true">
        ...
        <template v-slot:list-after-filter>
            <monitoring4-populate-assignment-form :data="formData" :dataKey="formDataKey" @success="refreshKey = refreshKey + 1"></monitoring4-populate-assignment-form>
        </template>
    </geko-base-crud>
...
import monitoring4PopulateConfig from "./Monitoring4PopulateComponent/monitoring4PopulateConfig";
import monitoring4PopulateDetail from "./monitoring4PopulateDetail.vue";
import Monitoring4PopulateAssignmentForm from "./Monitoring4PopulateComponent/Monitoring4PopulateAssignmentForm.vue";

export default {
    components: {
        LottieAnimation,
        monitoring4PopulateDetail,
        Monitoring4PopulateAssignmentForm
    },
    ...
    data() {
        return {
            user: {},
            refreshKey: 1,
            config: monitoring4PopulateConfig,
    ...
    methods: {
        ...
        async OnResetAssignedData(item){
            ...
            if (prompt.isConfirmed) {
                this.$_api.post('populate-monitoring/4-to-5/reset', {id: item.id,populate_no: item.populate_no})
                .then(() => {
        ...
        async onGenerateMonitoring(item){
            ...
            if (prompt.isConfirmed) {
                console.log(item)
                this.$_api.post('populate-monitoring/4-to-5/generate-monitoring', item)
                .then(() => {
        ...
        async onDeletePopulateData(item){
            ...
            if (prompt.isConfirmed) {
                this.$_api.post('populate-monitoring/4-to-5/delete', payload)
                .then(() => {
