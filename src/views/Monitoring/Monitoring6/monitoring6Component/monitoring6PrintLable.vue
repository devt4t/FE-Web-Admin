  ...
  <v-dialog v-model="isOpen" width="70%">
    <template v-slot:default="{ isActive }">
      <v-card class="mx-auto">
        <v-card-title>Print Label Pohon Monitoring 5 ({{ data.monitoring6_no }}), Petani: {{ data.farmer_name }}, Lahan:
        ...
        <form @submit.prevent="handleSubmit(onSubmitPrint())" autocomplete="off">
          <v-row>
            ...
            <v-col lg="12">
              <v-switch v-model="massPrint" :label="`Print Semua Data Pohon?`" @change="onMassPrintSwitch"></v-switch>
              <v-data-table v-if="massPrint == false" :headers="monitoring_trees_lable.table.header"
                :items="monitoring_trees_lable.table.items" :server-items-length="monitoring_trees_lable.totalRecord"
                :loading="monitoring_trees_lable.loading" :items-per-page="monitoring_trees_lable.perPage"
                class="elevation-1" @update:page="onChangePage" :page="monitoring_trees_lable.page"
...
export default {
  ...
  methods: {
    ...
    async GetData() {
      ...
      var payload = {
        'monitoring6_no': this.data.monitoring6_no,
        'limit': limit,
        'offset': offset,
        'search_value': this.monitoring_trees_lable.search
      }
      var MonitoringDetailUrl = "sixth-monitorings/main/print-lable";
    ...
    async onSubmitPrint() {
      ...
      const configFilename = `monitoring6-lable-${this.data.managementunits_name}-${this.data.lahan_no}-ff_${this.data.field_facilitators_name}-farmer_${this.data.farmers_name}-${moment().format("DMMYYYYHHmmss")}.pdf`;
      const axiosConfig = {
        method: "POST",
        url: configUrl,
        ...
        data: {
          data: this.monitoring_trees_lable.selected.map(data => {
            return {
              ...
              monitoring6_no: data.monitoring6_no,
        ...
        table: {
          header: [
            ...
            },
            {
              key: "monitoring6_no",
              sortable: false,
              text: "Nomor Monitoring 6",
              value: "monitoring6_no",
            },
            {
              key: "tree_name",
              sortable: false,
