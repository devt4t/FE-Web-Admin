<template>
    <geko-base-crud :config="config" :refreshKey="refreshKey" :hideUpdate="true" :hideDelete="true" :hideCreate="true">
        <template v-slot:detail-slave-raw="{ data }">
            <monitoring4-populate-detail :data="data"></monitoring4-populate-detail>
        </template>
        ...
        <template v-slot:list-after-filter>
            <monitoring4-populate-assignment-form :data="formData" :dataKey="formDataKey"
                @success="refreshKey = refreshKey + 1"></monitoring4-populate-assignment-form>
        </template>
    </geko-base-crud>
...
import monitoring6PopulateConfig from "./Monitoring6PopulateComponent/monitoring6PopulateConfig";
import monitoring6PopulateDetail from "./monitoring6PopulateDetail.vue";
import Monitoring6PopulateAssignmentForm from "./Monitoring6PopulateComponent/Monitoring6PopulateAssignmentForm.vue";

export default {
    components: {
        LottieAnimation,
        monitoring6PopulateDetail,
        Monitoring6PopulateAssignmentForm
    },
    name: "crud-monitoring4Populate",
    watch: {},
    data() {
        return {
            user: {},
            refreshKey: 1,
            config: monitoring6PopulateConfig,
    ...
    methods: {
        ...
        async OnResetAssignedData(item) {
            ...
            if (prompt.isConfirmed) {
                this.$_api.post('populate-monitoring/6-to-7/reset', { id: item.id, populate_no: item.populate_no })
                    .then(() => {
        ...
        async onGenerateMonitoring(item) {
            ...
            if (prompt.isConfirmed) {
                console.log(item)
                this.$_api.post('populate-monitoring/6-to-7/generate-monitoring', item)
                    .then(() => {
        ...
        async onDeletePopulateData(item) {
            ...
            var payload = {
                ...
                monitoring4_no: item.monitoring_no
            }
            if (prompt.isConfirmed) {
                this.$_api.post('populate-monitoring/6-to-7/delete', payload)
                    .then(() => {
