...
import monitoring5PopulateConfig from "./Monitoring5PopulateComponent/monitoring5PopulateConfig";
import monitoring5PopulateDetail from "./monitoring5PopulateDetail.vue";
import Monitoring5PopulateAssignmentForm from "./Monitoring5PopulateComponent/Monitoring5PopulateAssignmentForm.vue";

export default {
    components: {
        LottieAnimation,
        monitoring5PopulateDetail,
        Monitoring5PopulateAssignmentForm
    },
    name: "crud-monitoring5Populate",
    watch: {},
    data() {
        return {
            user: {},
            refreshKey: 1,
            config: monitoring5PopulateConfig,
    ...
    methods: {
        ...
        async OnResetAssignedData(item) {
            ...
            if (prompt.isConfirmed) {
                this.$_api.post('populate-monitoring/5-to-6/reset', { id: item.id, populate_no: item.populate_no })
                    .then(() => {
        ...
        async onGenerateMonitoring(item) {
            ...
            if (prompt.isConfirmed) {
                console.log(item)
                this.$_api.post('populate-monitoring/5-to-6/generate-monitoring', item)
                    .then(() => {
        ...
        async onDeletePopulateData(item) {
            ...
            if (prompt.isConfirmed) {
                this.$_api.post('populate-monitoring/5-to-6/delete', payload)
                    .then(() => {
