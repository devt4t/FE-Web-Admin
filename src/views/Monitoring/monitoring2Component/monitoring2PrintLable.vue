    ...
    <v-dialog v-model="isOpen" width="70%">
      <template v-slot:default="{ isActive }">
        <v-card class="mx-auto">
          <v-card-title>Print Label Pohon Monitoring 2 ({{ data.monitoring2_no }}), Petani: {{ data.farmer_name}}, Lahan: {{ data.lahan_no }}</v-card-title>
          <form @submit.prevent="handleSubmit(onSubmitPrint())" autocomplete="off">
              <v-row>
                  <v-col lg="12">
                      ...
                      <v-col lg="4" class="flex d-flex justify-center">
                          <v-progress-circular :rotate="360" :size="200" :width="20"
                              ...
                              <br>{{monitoring_trees_lable.selected.length}} / {{ monitoring_trees_lable.totalTrees }} - Max 1400</br>
                              <br>
                  ...
                  <v-col lg="12">
                      <v-data-table 
                      :headers="monitoring_trees_lable.table.header" 
                      :items="monitoring_trees_lable.table.items"
                      :server-items-length="monitoring_trees_lable.totalRecord" 
                      :loading="monitoring_trees_lable.loading"
                      :items-per-page="monitoring_trees_lable.perPage" 
                      class="elevation-1"
                          @update:page="onChangePage" 
                          :page="monitoring_trees_lable.page"
                          ...
                          :show-select="true" 
                          v-model="monitoring_trees_lable.selected"
                          :search="monitoring_trees_lable.search">
                          <template v-slot:top>
                              <v-text-field
                                  v-model="monitoring_trees_lable.search"
                  ...
                  <v-col lg="12">
                      <v-btn :disabled="monitoring_trees_lable.selected.length == 0 || monitoring_trees_lable.selected.length >= 1400" variant="success" type="submit">
