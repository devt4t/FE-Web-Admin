<template>
    <geko-base-crud :config="config" :refreshKey="refreshKey" :hideUpdate="true" :hideDelete="true" :hideCreate="true"
        @onExportExcel="onExportExcel($event)" :onExportExcelUrl="'monitoring2/main/export-excel'"
        :onExportExcelFileName="'Monitoring2'">
        <template v-slot:detail-slave-raw="{ data }">
            <monitoring2-detail-map :long="data.result.longitude" :lat="data.result.latitude" :section="`Monitoring2`"
        ...
        <template v-slot:list-bottom-action="{ item }">
            <v-btn variant="primary" small class="d-flex flex-row align-items-center mt-2"
                @click="onExportDetailSelectiom(item)" v-if="$_sys.isAllowed('monitoring-lookup')">
        ...
        <template v-slot:list-before-create>
            <monitoring2-export-selection :data="detailData"
                :dataKey="export_selection_key"></monitoring2-export-selection>
...
import populateModal from "./monitoring2Component/populateModalMonitoring2.vue";
