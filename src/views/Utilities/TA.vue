...
export default {
  ...
  computed: {
    ...
    optionsMUModal() {
      const items = this.itemsMu.map(val => {
        if (val.program_year && this.defaultItem.program_year) if (this.defaultItem.program_year.every(pro => val.program_year.split(',').includes(pro))) return val
        return null
      }).filter(n => n)
      if (this.defaultItem.program_year) if (!items.find(val => val.mu_no == this.defaultItem.mu_no)) this.defaultItem.mu_no = ''
      return items
    }
  },
  watch: {
  ...
  methods: {
    async initialize() {
      try {
        ...
        const response = await axios.get(
          this.BaseUrlGet + `GetTargetAreaAdmin?program_year=${this.localConfig.programYear}`,
          {
            headers: {
    ...
    async getKab() {
      try {
        ...
        // console.log(response.data.data.result);
        if (response.data.length != 0) {
          this.itemsKab = response.data.data.result.sort((a, b) => a.namaKabupaten.localeCompare(b.namaKabupaten));
        } else {
          console.log("Kosong");
        }
      } catch (error) {
    ...
    showAddModal() {
      ...
      this.getMu();
      this.defaultItem.id = "";
      if (this.localConfig.programYear != 'Semua') this.defaultItem.program_year = [this.localConfig.programYear]
