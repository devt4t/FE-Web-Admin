    ...
    <ValidationObserver ref="thirdForm" v-slot="{ handleSubmit }" v-show="form === 3">
      <form @submit.prevent="handleSubmit(onSubmit)" autocomplete="off">
        <v-row>
          ...
          <v-col md="6">
            <geko-input v-model="formData.next_event_contact_person_number" :item="{
          ...
          <v-col md="12">
            <div class="village-person-list">
              <v-row v-for="(person, i) in formData.ff_candidates" :key="'ff-' + i">
                ...
                <v-col md="1" class="d-flex flex-column justify-content-center"
                  ...
                  <button @click="removeFfCandidate(i)" v-if="formData.ff_candidates.length > 0">
