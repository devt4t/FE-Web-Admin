<template>
  <div class="rra-pra-form">
    ...
    <!-- <v-btn variant="light" @click="test()">Test</v-btn> -->
        ...
        <v-row>
          ...
          <v-col md="12">
            <div class="d-flex flex-row w-100" style="justify-content: flex-end">
              <v-btn variant="success" type="submit" @click="onSubmit(true)" class="mr-2 outline">
...
export default {
  ...
  data() {
    return {
      dusunJobs: [
        ...
        [
          "Profesi Lain (Peternakan, Buruh Peternakan, Pensiunan)",
          "job_others",
        ],
      ],
  ...
  methods: {
    async onSubmit(addPra = false) {
      try {
        ...
        for (const item of existingPlantPayload) {
          const isSuccess = await this.$_api
            .post("addRraExistingPlant_new", item)
            .then(() => {
              return true;
            })
            .catch((err) => {
        ...
        for (const item of communityInstitutionPayload) {
          const isSuccess = await this.$_api
            .post("addRraComunityInstitution_new", item)
            .then(() => {
              return true;
            })
            .catch((err) => {
        ...
        for (const item of organicPotentialsPayload) {
          const isSuccess = await this.$_api
            .post("addRraOrganicPotentials_new", item)
            .then(() => {
              return true;
            })
            .catch((err) => {
        ...
        for (const item of productionMarketingPayload) {
          const isSuccess = await this.$_api
            .post("addRraProductionMarketing_new", item)
            .then(() => {
              return true;
            })
            .catch((err) => {
        ...
        for (const item of innovativeFarmerPayload) {
          const isSuccess = await this.$_api
            .post("addRraInnovativeFarmer_new", item)
            .then(() => {
              return true;
            })
            .catch((err) => {
    ...
    test() {
      ...
      console.log('landscape desa', this.formData.borderlines);
    }
  },
  mounted() { },
