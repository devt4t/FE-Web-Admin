                    ...
                    <ValidationObserver ref="firstForm" v-slot="{ handleSubmit }">
                        <form @submit.prevent="handleSubmit(onSubmit)" autocomplete="off">
                            <v-row v-if="loading == false" lg="12" class="px-5 py-5">
                                ...
                                <v-col lg="6">
                                    <geko-input :disabled="!formData.target_area || !formData.program_year"
                                        v-model="formData.village" :item="{
                                            label: 'Desa',
                                            validation: ['required'],
                                            col_size: 6,
                                            type: 'select',
...
export default {
    ...
    methods: {
        ...
        onOpen() {
            ...
            // Convert comma-separated photo strings from DB into arrays for multiple upload component
            if (this.formData.photo_documentation1 && typeof this.formData.photo_documentation1 === 'string') {
                this.formData.photo_documentation1 = this.formData.photo_documentation1.split(',').filter(v => v.trim() !== '');
            } else if (!this.formData.photo_documentation1) {
                this.formData.photo_documentation1 = [];
            }

            if (this.formData.photo_documentation2 && typeof this.formData.photo_documentation2 === 'string') {
                this.formData.photo_documentation2 = this.formData.photo_documentation2.split(',').filter(v => v.trim() !== '');
            } else if (!this.formData.photo_documentation2) {
                this.formData.photo_documentation2 = [];
            }
        },
        ...
        onSubmit() {
            ...
            this.$_api
                .post("UpdateFormMinatMain_new", payload)
                .then(() => {
