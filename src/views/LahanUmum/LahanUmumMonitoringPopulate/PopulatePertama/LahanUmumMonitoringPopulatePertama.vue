<template>
    <geko-base-crud :config="config" :refreshKey="refreshKey" :hideUpdate="true" :hideDelete="true" :hideCreate="true">
        <template v-slot:detail-slave-raw="{ data }">
            <lahan-umum-monitoring-populate-pertama-detail :data="data"></lahan-umum-monitoring-populate-pertama-detail>
        </template>
        <template v-slot:list-bottom-action="{ item }">
            <v-btn variant="primary" small class="d-flex flex-row align-items-center mt-2"
                ...
                <span
                    v-if="(item.assigned_to == '-' && item.sampling == '-') || (item.assigned_to == null && item.sampling == null)">Lengkapi
                    Data Populasi</span>
        ...
        <template v-slot:list-after-filter>
            <lahan-umum-monitoring-populate-pertama-assignment-form :data="formData" :dataKey="formDataKey"
                @success="refreshKey = refreshKey + 1"></lahan-umum-monitoring-populate-pertama-assignment-form>
        </template>
    </geko-base-crud>
</template>
...
import lahanUmumMonitoringPopulatePertamaConfig from "./LahanUmumMonitoringPopulatePertamaConfig";
// import LahanUmumMonitoringPopulatePertamaDetail from "./LahanUmumMonitoringPopulatePertamaDetail.vue";
import LahanUmumMonitoringPopulatePertamaAssignmentForm from "./LahanUmumMonitoringPopulatePertamaAssignmentForm.vue";

export default {
    components: {
        LottieAnimation,
        // LahanUmumMonitoringPopulatePertamaDetail,
        LahanUmumMonitoringPopulatePertamaAssignmentForm
    },
    name: "crud-lahan-umum-monitoring-populate-pertama",
    watch: {},
    data() {
        return {
            user: {},
            refreshKey: 1,
            config: lahanUmumMonitoringPopulatePertamaConfig,
    ...
    methods: {
        ...
        async OnResetAssignedData(item) {
            ...
            if (prompt.isConfirmed) {
                this.$_api.post('general-land/populate-monitoring/1-to-2/reset-assignment', { id: item.id })
                    .then(() => {
        ...
        async onGenerateMonitoring(item) {
            const prompt = await this.$_alert.confirm('Generate Data Monitoring?', 'Harap Lakukan Dengan Teliti, Proses Ini Tidak Dapat Dikembalikan!', 'Ya, Generate!', 'Batal', true);
            if (prompt.isConfirmed) {
                this.$_api.post('general-land/populate-monitoring/1-to-2/generate-monitoring', item)
                    .then(() => {
                        this.$_alert.success('Berhasil Melakukan Generate Data Populasi Ke Monitoring 2!');
                        this.refreshKey += 1;
                    });
            }
        },
        async onDeletePopulateData(item) {
            ...
            if (prompt.isConfirmed) {
                this.$_api.post('general-land/populate-monitoring/1-to-2/delete', payload)
                    .then(() => {
