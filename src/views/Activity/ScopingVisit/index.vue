<template>
    <div>
        <!-- BreadCrumbs -->
        <v-breadcrumbs :dark="$store.state.theme == 'dark'" data-aos="fade-right" class="breadcrumbsmain"
            :items="breadcrumbs" divider=">" large></v-breadcrumbs>
        <!-- Modals -->
        <FormModal :show="modals.form.show" :id="modals.form.data" :programYear="localConfig.programYear"
            @action="$v => modalActions($v)" @swal="$v => swalActions($v)" :key="modals.form.key" />
        <DetailModal :show="modals.detail.show" :id="modals.detail.data" :programYear="localConfig.programYear"
            @action="$v => modalActions($v)" @swal="$v => swalActions($v)" :key="modals.detail.key" />
        <!-- Main Table -->
        <div class="px-4">
            <v-alert type="info">Geko V2.0<br />Modul Scooping Visit bisa diakses melalui menu <strong
                    class="cursor-pointer" @click="$router.push({
        ...
        <v-data-table data-aos="fade-up" data-aos-delay="200" multi-sort :headers="table.headers" :items="table.items"
            :search="table.search" :loading="table.loading.show" :loading-text="table.loading.text"
            ...
            <template v-slot:top>
                <v-row class="my-2 mx-2 mx-lg-3 align-center">
                    ...
                    ></v-select> -->
                    <!-- Search Field -->
                    <v-text-field color="green" hide-details dense outlined rounded class="mx-auto mb-2 mb-lg-0"
                        label="Pencarian" placeholder="Start type to search..." append-icon="mdi-magnify"
                        v-model="table.search"></v-text-field>
                    <v-divider class="mx-2 d-none d-lg-block"></v-divider>
                    <div class="mx-auto">
                        <v-btn v-if="false" color="green white--text"
                            :disabled="table.loading.show || table.items.length == 0" rounded class="mr-2"
                            @click="() => exportExcel()" disabled>
                            <v-icon class="mr-1">mdi-microsoft-excel</v-icon>
                            Export
                        </v-btn>
                        <v-btn v-if="false" color="info" rounded class="pl-2"
                            @click="() => { modals.form.show = true; modals.form.key += 1 }">
